...
from gym_pybullet_drones.envs.constrained_environment_no_kf import UAV2DAvoidSimple1NoKF
...
def make_env(seed: int, rank: int, stage: int = 1):
    def _init():
        env = UAV2DAvoidSimple1NoKF(render_mode=None)
        env = Monitor(env)
...
def build_vec_env(seed: int, n_envs: int, backend: str, monitor_file: Path, stage: int = 1):
...
class PaperEvalCallback(BaseCallback):
    def __init__(
        ...
        self.max_curriculum_stages = 4
        self.curriculum_gate = 0.90
        self.curriculum_consecutive = 3
...
def train_one_algorithm(algo: str, args: argparse.Namespace, backend: str) -> Path:
    run_dir = make_run_dir(args, algo)
